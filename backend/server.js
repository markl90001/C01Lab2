...
// Retrieves all notes belonging to a user
app.get("/getAllNotes", express.json(), async(req, res) => {
  try {
    ...
    jwt.verify(token, "secret-key", async (err, decoded) => {
      ...
      // Find notes with corresponding username
      const collection = db.collection(COLLECTIONS.notes);
...
app.patch("/editNote/:noteId", express.json(), async (req, res) => {
  try {
    ...
    if (!title && !content) {
      return res
        .status(400)
        .json({ error: "One of title or content is required." });
    }
    ...
    jwt.verify(token, "secret-key", async (err, decoded) => {
      ...
      // Update the note
      if (title) await collection.updateOne({_id: new ObjectId(noteId)}, {$set: {title: title}});
      if (content) await collection.updateOne({_id: new ObjectId(noteId)}, {$set: {content: content}});
